<script>
  ...
  import { loadExternalImage, loadImageFromAppFolder } from "$lib/fs";
  ...
  async function loadCards(reset = false) {
    ...
    for (const card of rows) {
      ...
      // });
      loadImageFromAppFolder(card.front_image_en, cardno_filter).then((url) => {
        imageUrls[card.card_no] = url ? url : card.front_image_en;
      });
    }
...
{#if filterVisible}
  ...
  <div class="filter-modal" role="dialog" tabindex="0">
        ...
        <div class="filter-section">
          <div class="filter-title">
            ...
            <div class="filter-mode">
              <select
                id={key}
                onchange={(e) => changeMode(key, e.target.value)}
          ...
          <div class="filter-options">
            {#each values as v}
              <button
                type="button"
                class="choice {queryOptions[key]?.includes(v) ? 'active' : ''}"
                aria-pressed={queryOptions[key]?.includes(v) ? "true" : "false"}
    ...
    <div class="filter-footer">
      <button type="button" class="clear" tabindex="0" onclick={clearAllFilters}
        >清空</button
      >
      <button
        type="button"
