...
export async function getDB() {
  if (!db) {
    ...
    await db.execute(`
      INSERT OR IGNORE INTO setting_table (name, value, options) VALUES
        ('region', '默认', '默认,中国')
    `);
...
export const getSettings = async () => {
  ...
  return await db.select("SELECT * FROM setting_table");
};
...
export async function getDistinctFilters() {
  ...
  // JSON 数组字段（keywords）
  const rows = await db.select(
    "SELECT keywords FROM cards WHERE keywords IS NOT NULL",
  );
...
export async function searchCards({
  ...
  if (series?.length) {
    switch (selectModes["series"]) {
      case "excludeSelected":
        // 排除：集合中不能出现任何选中
        whereClauses.push(
          `series_name NOT IN (${series.map(() => "?").join(",")})`,
        );
        params.push(...series);
        break;
      ...
      default:
        // 任意匹配
        whereClauses.push(
          `series_name IN (${series.map(() => "?").join(",")})`,
        );
        params.push(...series);
        break;
    }
  ...
  if (type?.length) {
    switch (selectModes["type"]) {
      case "excludeSelected":
        // 排除：集合中不能出现任何选中
        whereClauses.push(
          `card_category_name NOT IN (${type.map(() => "?").join(",")})`,
        );
        params.push(...type);
        break;
      ...
      default:
        // 任意匹配
        whereClauses.push(
          `card_category_name IN (${type.map(() => "?").join(",")})`,
        );
        params.push(...type);
        break;
    }
  ...
  if (rarity?.length) {
    switch (selectModes["rarity"]) {
      case "excludeSelected":
        // 排除：集合中不能出现任何选中
        whereClauses.push(
          `rarity_name NOT IN (${rarity.map(() => "?").join(",")})`,
        );
        params.push(...rarity);
        break;
      ...
      default:
        // 任意匹配
        whereClauses.push(
          `rarity_name IN (${rarity.map(() => "?").join(",")})`,
        );
        params.push(...rarity);
        break;
    }
  ...
  if (tag?.length) {
    ...
    switch (selectModes["tag"]) {
      case "onlySelected":
        ...
        whereClauses.push(
          `(LENGTH(cards.tag) - LENGTH(REPLACE(cards.tag, '|', ''))) = ?`,
        );
  ...
  if (color?.length) {
    ...
    switch (selectModes["color"]) {
      ...
      case "includeSelected":
        // 每个选中都必须存在 json 数组里
        const mustIncludeAllConditions = color
          .map(
            () => `
          EXISTS (
            SELECT 1 FROM json_each(cards.card_color_list)
  ...
  if (keyword?.length) {
    ...
    switch (selectModes["keyword"]) {
      ...
      case "includeSelected":
        ...
        const mustIncludeAllConditions = keyword
          .map(
            () => `
          EXISTS (
            SELECT 1 FROM json_each(cards.keywords)
  ...
  const totalRows = await db.select(
    `SELECT COUNT(*) AS total FROM cards ${whereSQL}`,
    params,
  );
  ...
  let orderByRules = [];
  if (orderBy === "card_no") {
    orderByRules = [{ col: "card_no", asc: isAsc }];
  } else {
    orderByRules = [
...
export async function loadDeck(deckId) {
  ...
  const [deck] = await db.select("SELECT * FROM decks WHERE id = ?", [deckId]);
...
export const getTableState = async (tables = []) => {
  ...
  const rows = await db.select(sql, tables);
  if (rows.length === 0) return null;
  return rows;
};
